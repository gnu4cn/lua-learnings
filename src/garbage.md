...
由于 Lua 会在终结对象上打上标记，故每个对象的终结器都只会运行一次。如果一个对象直到程序结束时才被收集，那么当整个 Lua 状态关闭时，Lua 将调用它的终结器。最后一个特性允许在 Lua 中使用一种 atexit 函数，即在程序结束前立即运行的函数。我们所要做的就是创建一个带有终结器的表格，并将其固定在某个地方，例如全局变量中：
