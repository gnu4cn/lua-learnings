...
当然，外部表提供了一种将属性映射到对象的理想方式。这就是我们在 [“双重表示法”](oop.md#双重表示) 小节中，所说的 *双重表示法，dual representation*。我们将对象用作键，将其属性作为值。由于 Lua 允许我们使用任何类型对象作为键，因此外部表就可以保存任何类型对象的属性。此外，外部表中保存的属性，还不会干扰其他对象，并且可以像表本身一样私有。
